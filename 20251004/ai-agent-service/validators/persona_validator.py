# tools/validators/persona_validator.py
from typing import List, Dict, Any, Optional, Union
from pydantic import BaseModel, Field, ValidationError, field_validator

class PersonaModel(BaseModel):
    name: str = Field(..., min_length=1, description="페르소나의 이름 (빈 값 불가)")
    role: str = Field(..., min_length=1, description="페르소나의 역할을 나타내는 한 줄 요약 (빈 값 불가)")
    demographics: str = Field(..., min_length=1, description="인구 통계 정보 (빈 값 불가)")
    behavioral_traits: List[str] = Field(..., min_items=1, description="주요 행동 특성 목록")
    needs_and_goals: List[str] = Field(..., min_items=1, description="핵심 니즈와 목표 목록")
    pain_points: List[str] = Field(..., min_items=1, description="핵심 불편함 목록")


def validate_personas(personas_data: Union[List[Dict[str, Any]], Dict[str, Any]]) -> List[Dict[str, Any]]:
    """
    페르소나 데이터 유효성 검사
    
    Args:
        personas_data: 리스트 또는 {'personas': [...]} 형태의 딕셔너리
    
    Returns:
        검증된 페르소나 리스트
    """
    
    # 🔥 딕셔너리인 경우 personas 키에서 추출
    if isinstance(personas_data, dict):
        if "personas" in personas_data:
            personas_data = personas_data["personas"]
        else:
            print(f"--- 페르소나 유효성 검사 경고 ---")
            print(f"딕셔너리에 'personas' 키가 없습니다: {personas_data}")
            print("---------------------------------")
            return []
    
    if not isinstance(personas_data, list):
        print(f"--- 페르소나 유효성 검사 경고 ---")
        print(f"입력 데이터가 리스트가 아닙니다: {personas_data}")
        print("---------------------------------")
        return []

    validated_personas = []
    for p_data in personas_data:
        try:
            # Pydantic 모델로 데이터 유효성 검사 시도
            validated_persona = PersonaModel(**p_data)
            # 유효성 검사를 통과하면, 다시 딕셔너리 형태로 변환하여 리스트에 추가
            validated_personas.append(validated_persona.model_dump())
        except ValidationError as e:
            # 유효성 검사 실패 시, 어떤 페르소나에서 어떤 오류가 났는지 상세히 로그를 남깁니다.
            print(f"--- 페르소나 유효성 검사 실패 ---")
            print(f"오류 발생 페르소나 데이터: {p_data}")
            print(f"상세 오류 내용: {e}")
            print("---------------------------------")
            # 실패한 페르소나는 결과에 포함시키지 않습니다.
            continue
            
    return validated_personas