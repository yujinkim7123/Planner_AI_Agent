...
def build_modify_data_plan_prompt(
    ...
    """기존 데이터 기획안 수정을 위한 프롬프트를 더 명확한 지시사항과 함께 생성합니다."""

    # 기존 기획안을 JSON 문자열로 변환하여 프롬프트에 포함
    existing_plan_str = json.dumps(existing_plan, indent=2, ensure_ascii=False)

    prompt = f"""
    당신은 뛰어난 데이터 기획 전문가입니다.
    주어진 '기존 데이터 기획안'을 '수정 요청사항'에 맞게 수정해주세요.

    [서비스 아이디어]
    - 서비스명: {service_idea.get("name", "N/A")}
    - 상세 설명: {service_idea.get("description", "N/A")}
    ...
    최종 지시사항
    1.'기존 데이터 기획안'의 전체 구조와 형식을 반드시 유지해야 합니다.

    2. 위 구조를 기반으로 '수정 요청사항'을 반영하여 내용을 추가하거나 변경해주세요.

    3. 수정 요청이 없는 부분은 기존 내용 그대로 유지해야 합니다.
