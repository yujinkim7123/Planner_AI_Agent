...
def _build_service_creation_prompt(persona: dict, product_type: str | None, device_columns: dict, feature_docs: list, sensor_columns: list, num_ideas: int) -> str:
    ...
    # print(f"📌 prompt ")
    # print(persona)
    # print(device_columns)
    # print(feature_docs)
    # print(sensor_columns)
    # print("pass")
    # print(json.dumps(device_columns, ensure_ascii=False))
    # print("pass")
    # print(json.dumps(feature_docs, ensure_ascii=False))
    # print("pass")
    # print(json.dumps(sensor_columns, ensure_ascii=False))
    # print("pass")
    ...
    - 제품 상세 데이터 필드: {json.dumps(device_columns, ensure_ascii=False, default=_convert_datetime_to_str)}
    - 관련 기능 문서 요약: {json.dumps(feature_docs, ensure_ascii=False, default=_convert_datetime_to_str)}
    - **연관 센서 데이터 (샘플): {json.dumps(sensor_columns, ensure_ascii=False, default=_convert_datetime_to_str)}**
    """
    else:
        product_context_prompt = """
    ### [기존 제품 및 기능 정보]
...
def create_service_ideas(workspace: dict, persona_name: str, num_ideas: int = 3):
    ...
    try:
        ...
        workspace["artifacts"]["service_ideas"] = existing_ideas_list
...
def modify_service_ideas(workspace: dict, modification_request: str):
    ...
    ### [참고용 기술 데이터]
    - 제품 상세 데이터 필드: {json.dumps(device_columns,  ensure_ascii=False, default=_convert_datetime_to_str)}
    - 관련 기능 문서 요약: {json.dumps(product_docs,  ensure_ascii=False, default=_convert_datetime_to_str)}
    - 연관 센서 데이터 (샘플): {json.dumps(sensor_columns,  ensure_ascii=False, default=_convert_datetime_to_str)}
    - 제품군 : {product_type}
    """
    ...
    try:
        ...
        existing_ideas = existing_ideas_obj
