    ...
    <div className="flex flex-col items-center justify-center h-full bg-gray-50 dark:bg-gray-900 p-8">
      <div className="w-full max-w-2xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 p-6 border rounded-lg bg-white dark:bg-gray-800 shadow-xl">
          ...
          <div>
            <label htmlFor="key_behaviors" className="block text-sm font-medium mb-1">핵심 행동 (Key Behaviors)</label>
            <textarea id="key_behaviors" value={formData.key_behaviors} onChange={handleChange} placeholder="핵심 행동을 한 줄에 하나씩 입력해주세요. 예: 아이 옷은 반드시 살균 기능으로 관리" required rows={3} className="w-full p-2 border rounded-md dark:bg-gray-700 dark:border-gray-600"/>
          </div>
          <div>
