...
def create_data_plan_for_service(workspace: dict, service_name: str = None, product_type: str = None):
    ...
    if service_name:
        all_ideas = artifacts.get("service_ideas", {})
    ...
    if final_product_type:
        ...
        - **기존 제품 상세 데이터 필드:** {json.dumps(device_columns, ensure_ascii=False, default=_convert_datetime_to_str)}
        - **관련 제품 기능 문서 (요약):** {json.dumps(product_docs, ensure_ascii=False, default=_convert_datetime_to_str)}
        - **관련 센서 데이터 (샘플):** {json.dumps(sensor_columns, ensure_ascii=False, default=_convert_datetime_to_str)}
        """
    else:
